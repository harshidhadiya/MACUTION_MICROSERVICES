...
public sealed class RpcAdminPendingConsumer : BackgroundService
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);
    private readonly IRabbitMqConnection _connection;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<RpcAdminPendingConsumer> _logger;
    private IModel? _channel;

    public RpcAdminPendingConsumer(
        IRabbitMqConnection connection,
        IServiceScopeFactory scopeFactory,
        ILogger<RpcAdminPendingConsumer> logger)
    {
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _channel.QueueDeclare(RpcConstants.AdminPendingQueue, durable: true, exclusive: false, autoDelete: false);
        _channel.QueueBind(RpcConstants.AdminPendingQueue, RpcConstants.RpcExchange, RpcConstants.AdminPendingQueue);
        _channel.BasicQos(0, 10, false);

        var consumer = new AsyncEventingBasicConsumer(_channel);
        consumer.Received += async (_, ea) => await OnReceivedAsync(ea);
        _channel.BasicConsume(RpcConstants.AdminPendingQueue, autoAck: false, consumer: consumer);
    ...
    private async Task OnReceivedAsync(BasicDeliverEventArgs ea)
    {
        ...
        try
        {
            ...
            var list = db.REQUESTS.Where(r => !r.VerifiedByAdmin).AsNoTracking().ToList();
        ...
        {
            _logger.LogError(ex, "Rpc admin pending error");
